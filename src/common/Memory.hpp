...
namespace sapphire {
    ...
    template <typename T>
    constexpr T &dAccess(void *obj, uintptr_t offset) noexcept {
    ...
    template <typename T>
    constexpr const T &dAccess(const void *obj, uintptr_t offset) noexcept {
        return *reinterpret_cast<T *>(reinterpret_cast<uintptr_t>(obj) + offset);
    }
    ...
    constexpr T getVftable(void *obj) noexcept {
        return std::bit_cast<T>(*reinterpret_cast<void ***>(obj));
    }

    template <typename T = void **>
    constexpr T getVftable(const void *obj) noexcept {
    ...
    constexpr uintptr_t toFuncAddr(Fn addr) noexcept {
    ...
    constexpr uintptr_t toFuncAddr(uintptr_t addr) noexcept {
        return addr;
    }

    template <typename Fn>
        requires(std::is_member_function_pointer_v<Fn> && sizeof(Fn) == 2 * sizeof(uintptr_t))
    constexpr Fn toMemberFunc(uintptr_t addr) noexcept {
        struct member_func_ptr {
            uintptr_t addr;
            uintptr_t offset;
        };
    ...
    Ret vCall(void *obj, size_t fnIdx, Args &&...args) noexcept {
    ...
    template <typename Ret = void, typename... Args>
    Ret vCall(const void *obj, size_t fnIdx, Args &&...args) noexcept {
    ...
    template <typename Ret = void, typename... Args>
    Ret vCallOffset(void *obj, intptr_t offset, size_t fnIdx, Args &&...args) noexcept {
        return reinterpret_cast<Ret (*)(void *, Args &&...)>(
            getVftable(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(obj) + offset))[fnIdx]
        )(obj, std::forward<Args>(args)...);
    }

    inline uintptr_t ripRel(uintptr_t instrcutionAddr, uint32_t offset, uint32_t insLen) noexcept {
    ...
    inline uintptr_t ripRel(uintptr_t instrcutionAddr, InstType opType) noexcept {
    ...
    template <typename T>
    constexpr T ripRel(T instrcutionAddr, InstType opType) noexcept {
    ...
    decltype(auto) operator->*(T *obj, Field<TField, Offset>) noexcept {
