...
public:
     ...
     *        这通常用于ImGui窗口获得鼠标焦点时。
     */
    void requestAllMouseBlock() { mBlockMouseStatus |= MouseBlockStatus::BlockAll; }
...
private:
    ...
    uint8_t           mBlockMouseStatus = MouseBlockStatus::NoBlock; // 鼠标

    static constexpr size_t KeyCodeCount =
        static_cast<size_t>(KeyCode::NamedKey_END) - static_cast<size_t>(KeyCode::NamedKey_BEGIN);

    // 不包含修饰键
    std::bitset<KeyCodeCount> mBlockedKeysAndButtons;
