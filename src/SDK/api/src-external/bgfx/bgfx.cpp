#include "bgfx.h"
#include "SDK/core/SymbolResolver.h"

bgfx::Context *bgfx::s_ctx = sapphire::bind::data<
    sapphire::deRefMov | "\x48\x8B\x15\x00\x00\x00\x00\x45\x33\xDB"_sig,
    &bgfx::s_ctx,
    bgfx::Context *>();

bx::AllocatorI *bgfx::g_allocator = sapphire::bind::data<
    sapphire::deRefMov | "\x48\x8B\x0D\x00\x00\x00\x00\x81\xC7"_sig,
    &bgfx::g_allocator,
    bx::AllocatorI *>();

void bgfx::VertexDecl::end() {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x48\x89\x74\x24\x00\x55\x48\x8B\xEC\x48\x83\xEC\x00\x45\x33\xC9"_sig, // 1.21.50
        &VertexDecl::end>;
    (this->*Bind::origin)();
}

uint16_t bgfx::Encoder::setScissor(uint16_t _x, uint16_t _y, uint16_t _width, uint16_t _height) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x48\x89\x7C\x24\x00\x4C\x8B\x11\x0F\xB7\xFA"_sig, // 1.21.50
        &Encoder::setScissor>;
    return (this->*Bind::origin)(_x, _y, _width, _height);
}

void bgfx::Encoder::setVertexBuffer(
    uint8_t                         _stream,
    bgfx::DynamicVertexBufferHandle _handle,
    uint32_t                        _startVertex,
    uint32_t                        _numVertices,
    bgfx::VertexDeclHandle          _decl,
    uint32_t                        _stride
) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x48\x89\x74\x24\x00\x48\x89\x7C\x24\x00\x41\x0F\xB7\xC0"_sig, // 1.21.50
        &Encoder::setVertexBuffer>;
    (this->*Bind::origin)(_stream, _handle, _startVertex, _numVertices, _decl, _stride);
}

int bgfx::init(const bgfx::Init &_init, bool a2) {
    using Bind = sapphire::bind::Fn<
        "\x40\x53\x56\x57\x48\x83\xEC\x00\x0F\xB6\xF2"_sig, // 1.21.50
        &init>;
    return (Bind::origin)(_init, a2);
}

int bgfx::initCaps(const bgfx::Init &_init, int &a2) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x55\x56\x57\x48\x83\xEC\x00\x48\x8B\x05\x00\x00\x00\x00\x48\x33\xC4\x48\x89\x44\x24\x00\x48\x8B\xFA\x48\x8B\xD9\x48\x83\x3D"_sig, // 1.21.50
        &initCaps>;
    return (Bind::origin)(_init, a2);
}

bgfx::DynamicVertexBufferHandle bgfx::createDynamicVertexBuffer(
    const bgfx::Memory     *_mem,
    const bgfx::VertexDecl &_decl,
    uint16_t                _flags
) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x48\x89\x6C\x24\x00\x56\x57\x41\x56\x48\x83\xEC\x00\x41\x0F\xB7\xF8"_sig, // 1.21.50
        &createDynamicVertexBuffer>;
    return (Bind::origin)(_mem, _decl, _flags);
}

bgfx::DynamicIndexBufferHandle bgfx::createDynamicIndexBuffer(const bgfx::Memory *_mem, uint16_t _flags) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x55\x56\x57\x41\x56\x41\x57\x48\x83\xEC\x00\x0F\xB7\xFA"_sig, // 1.21.50
        createDynamicIndexBuffer>;
    return (Bind::origin)(_mem, _flags);
}

void bgfx::allocInstanceDataBuffer(
    bgfx::InstanceDataBuffer *_idb,
    uint32_t                  _num,
    uint16_t                  _stride
) {
    using Bind = sapphire::bind::Fn<
        "\x48\x89\x5C\x24\x00\x48\x89\x6C\x24\x00\x56\x57\x41\x56\x48\x83\xEC\x00\x48\x8B\x35\x00\x00\x00\x00\x4C\x8B\xF1"_sig, // 1.21.50
        allocInstanceDataBuffer>;
    (Bind::origin)(_idb, _num, _stride);
}

void bgfx::updateOffset(
    bgfx::DynamicVertexBufferHandle _handle,
    uint32_t                        _offset,
    uint32_t                        _declStride,
    const bgfx::Memory             *_mem
) {
    using Bind = sapphire::bind::Fn<
        "\x40\x53\x55\x56\x57\x41\x56\x41\x57\x48\x83\xEC\x00\x48\x8B\x05\x00\x00\x00\x00\x48\x33\xC4\x48\x89\x44\x24\x00\x48\x8B\x2D"_sig, // 1.21.50
        updateOffset>;
    (Bind::origin)(_handle, _offset, _declStride, _mem);
}